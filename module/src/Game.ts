...
export class Game {
    /**存放形状对象的数组 */
    private circleArray:Array<Shape>;
    /**绘图画布 */
     ...
     * @param merged 是否碰撞合并
     */
    constructor(initCount:number, ctx:CanvasRenderingContext2D, mSize:number, sizeSD:number, horizenSpeedMax:number, verticalSpeedMax:number, recover:number, floatRate:number, merged:boolean) {
        this.circleArray = [];
        this.ctx = ctx;
        this.merged = merged;
        if (floatRate > 1 || floatRate < 0) floatRate = 1;
        this.recover = recover;
        for (var i = 0; i < initCount; i++) {
            this.circleArray.push(new Circle(
                Math.random() * ctx.canvas.width, 
                Math.random() * ctx.canvas.height, 
                (Math.random() - 0.5) * horizenSpeedMax * 2, 
                (Math.random() - 0.5) * verticalSpeedMax * 2, 
                mSize + (Math.random() - 0.5) * sizeSD * 2,
                randomColor(),
                this.recover,
                Math.random() > floatRate,
                ctx));
        }
    }
    /**
